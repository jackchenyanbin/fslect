(function($) {
    $.fn.fSelect = function(options) {
        ...
        fSelect.prototype = {
            create: function() {
                ...
                if (this.settings.showButton){
                    var BtnFixed = '<div class="btnFixed">' +
                        '<button class="buttonDefault" id="selectAllBtn" type="button">全 选</button><button class="buttonDefault" id="invertBtn" type="button">反 选</button>' +
                        '<button class="buttonDefault" id="clearAllBtn" type="button">全不选</button><button class="buttonDefault" id="closeBtn" type="button">关 闭</button>' +
                        '</div>';
            ...
            reload: function() {
                //如果设置显示搜索框，执行下面方法
                var dropdown = this.$wrap.find('.fs-dropdown');
                if (this.settings.showSearch && !dropdown.children().hasClass('fs-search')) {
                    var search = '<div class="fs-search"><input type="search" placeholder="' + this.settings.searchText + '" /></div>';
                    dropdown.prepend(search);
                }
                var choices = this.buildOptions(this.$select,this.settings);
                //获取到所有option选项
            ...
            destroy: function() {
                this.$wrap.find('.fs-wrap').remove();
                this.$wrap.find('.multiple').remove();
            ...
            buildOptions: function($element,settings) {
                var $this = this;
                var choices = '';
                $element.children().each(function(i, el) {
                    ...
                    if ('optgroup' == $el.prop('nodeName').toLowerCase()) {
                        ...
                        if(i==0 && isNotBlank($el.prop('value'))){
                            choices += '<div class="fs-option selected" data-value=""><span class="fs-checkbox"><i></i></span><div class="fs-option-label">' + settings.placeholder + '</div></div>';
                        }else{
        ...
        return this.each(function() {
            ...
            if (typeof settings == 'string' && data!=null && data !=undefined) {
                data[settings]();
            }
        });
    };
    ...
    $(document).on('click','#clearAllBtn', function( ) {
        ...
        if ($wrap.hasClass('multiple')) {
            var selected = [];
            $wrap.find(".fs-option:not(:first)").removeClass('selected');
            $wrap.find('.fs-option.selected').each(function(i, el) {
                selected.push($(el).attr('data-value'));
            });
        }
